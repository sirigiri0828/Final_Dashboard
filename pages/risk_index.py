...
def risk_index(year):
    ...
    req_cols_for_inspection=["MINE_ID","SUM_TOTAL_INSP_HOURS_","CAL_YR"]
    ...
    print(df_inspections) 
    groupby_df1 = pd.DataFrame(df_inspections.groupby(risk_index_level)["SUM_TOTAL_INSP_HOURS_"].sum())
    groupby_df1.columns=["SUM_TOTAL_INSP_HOURS_"]
    ...
    Table_1.info()
    Table_1 = Table_1.rename(columns = {True:'No. S&S'})
    Table1 = Table_1.filter(["MINE_ID","CAL_YR","AVG_ANNUAL_EMPL","ANNUAL_HRS","Citation","No. S&S","Order","SUM_TOTAL_INSP_HOURS_","FATALITY","NO_AFFECTED","PROPOSED_PENALTY"]).reset_index(drop=True)
    Table1.info()
    ...
    Table_2["C/100"] = (Table1["Citation"] * 100)/ Table1["SUM_TOTAL_INSP_HOURS_"]
    Table_2["SS/100"] = (Table1["No. S&S"] * 100)/ Table1["SUM_TOTAL_INSP_HOURS_"]
    Table_2["O/100"] = (Table1["Order"] * 100)/ Table1["SUM_TOTAL_INSP_HOURS_"]
